...
function App() {
  ...
  const theme = getThemeColors(currentCafe.themeColor);
  const themeStyle = getThemeStyle(currentCafe.themeColor);

  return (
    <div
      className="min-h-screen bg-[#020204] text-white overflow-hidden relative"
      style={{
        ...themeStyle,
        // @ts-ignore - CSS custom property for selection color
        '--selection-color': `rgba(${theme.primaryRgb}, 0.3)`,
      } as React.CSSProperties}
    >
      {/* Selection color style */}
      <style>{`
        ::selection { background: rgba(${theme.primaryRgb}, 0.3); }
      `}</style>

      {/* Ambient Background Effects */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        {/* Animated Liquid Blobs — use theme color */}
        <div
          className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] rounded-full mix-blend-screen filter blur-[120px] animate-blob"
          style={{ backgroundColor: `rgba(${theme.primaryRgb}, 0.05)` }}
        />
      ...
      <div className="relative z-10">
        {/* Ambient crossfading cafe photos as header background */}
        {currentCafe.ambientImages && currentCafe.ambientImages.length > 0 && (
          <AmbientHero images={currentCafe.ambientImages} interval={5000} />
        )}
        ...
        <main className="space-y-12">
          ...
          {/* "The Space" gallery — only shown if cafe has photos */}
        ...
        <footer className="py-12 text-center text-white/40 text-xs font-mono border-t border-white/10 bg-black/30 backdrop-blur-md">
          <div className="flex flex-col items-center justify-center gap-2">
            ...
            <a
              ...
              style={{ color: `rgba(${theme.primaryRgb}, 0.7)` }}
              onMouseEnter={(e) => e.currentTarget.style.color = `rgb(${theme.primaryRgb})`}
              onMouseLeave={(e) => e.currentTarget.style.color = `rgba(${theme.primaryRgb}, 0.7)`}
            >
